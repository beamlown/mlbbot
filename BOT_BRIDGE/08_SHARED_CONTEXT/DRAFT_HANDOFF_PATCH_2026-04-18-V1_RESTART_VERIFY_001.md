# DRAFT HANDOFF: PATCH_2026-04-18-V1_RESTART_VERIFY_001

Auditor-drafted proposal. Manager must review and promote to `05_INBOX_FROM_MANAGER/HANDOFF_PATCH_2026-04-18-V1_RESTART_VERIFY_001.md` before any worker claims it.

## Status: DRAFT (awaiting manager endorsement)

**Title**: Restart bots for patch 2026-04-18-v1 and verify new runtime is live
**Priority**: HIGH
**Subsystem**: runtime / launcher
**Issued (draft)**: 2026-04-18 (auditor proposal)
...
- Do not re-dispatch PAPER_BRIDGE_001 as part of this task (separate ticket, different worker path).
...
## Manager note (template — fill in on promotion)

On promoting this draft to a real HANDOFF, verify:
- That the patch has in fact been audited SHIP (check `AUDIT_2026-04-18-v1.md`).
- That no other restart task is currently in flight.
- That the canonical launcher path is documented in the runtime rules (if not, attach the path in the HANDOFF before issue).
